<script lang="ts">
  ...
  import { Users, Activity, Search, ChevronRight, RotateCw } from 'lucide-svelte';
  ...
  async function loadLogs() {
    try {
      loading = true;
      const records = await pb.collection('system_logs').getList(1, 200, { sort: '-created' });
  ...
  function getRowBg(level: ActivityLog['level']) {
    switch (level) {
      ...
      default: return 'bg-white hover:bg-muted/50';
    }
  }
</script>
...
<div class="flex min-h-screen bg-background">
  ...
  {#snippet actionButton()}
    <button onclick={loadLogs} class="p-2 hover:bg-muted rounded-lg transition-colors" title="Refresh">
          ...
          <table class="w-full">
            ...
            <tbody>
              {#each filteredLogs as log (log.id)}
                <!-- Main Row -->
                <tr
                  class="border-b border-border transition-colors cursor-pointer {getRowBg(log.level)}"
                ...
                >
                  ...
                  <td class="px-4 py-3">
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border {getLevelBadgeColor(log.level)}">
                  ...
                  <td class="px-4 py-3">
                    ...
                    {#if log.details}
                      <div class="text-xs text-muted-foreground mt-1">{log.details}</div>
                    {/if}
                  </td>
                  <td class="px-4 py-3">
                    <span class="font-mono text-sm px-2 py-1 rounded {log.statusCode < 300 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
                      {log.statusCode}
                    </span>
                  </td>
                  <td class="px-4 py-3">
                    <div class="text-sm">
                      ...
                      <div class="text-muted-foreground font-mono text-xs">{log.createdAt.split(' ')[1]}</div>
                    </div>
                  </td>
                  <td class="px-4 py-3">
                    <ChevronRight class="w-4 h-4 text-muted-foreground transition-transform {expandedLog === log.id ? 'rotate-90' : ''}" />
                  </td>
                </tr>

                <!-- Expanded Details -->
                {#if expandedLog === log.id}
                  <tr transition:fade={{ duration: 150 }} class={getRowBg(log.level)}>
                    <td colspan="6" class="px-4 py-4 border-b border-border">
                      <div class="bg-muted/50 rounded-lg p-4 font-mono text-sm">
                        <div class="grid grid-cols-2 gap-4">
                          <div>
                            <span class="text-muted-foreground">Full Message:</span>
                            <div class="mt-1">{log.message}</div>
                          </div>
                          <div>
                            <span class="text-muted-foreground">Details:</span>
                            <div class="mt-1">{log.details || 'No additional details'}</div>
                          </div>
                          <div>
