<script lang="ts">
  ...
  let topProducts = $state<
    Array<{ product_name: string; total_qty: number; total_rev: number }>
  >([]);
  let paymentMethods = $state<
    Array<{ method: string; total: number; count: number }>
  >([]);
  ...
  let avgTransaction = $derived(
    totalTransactions ? Math.round(totalSales / totalTransactions) : 0,
  );
  ...
  <FluidLayout>
    ...
    {:else}
      <PageHeader
        ...
        subtitle="Detailed sales insights and trends"
        icon={TrendingUp}
      />
      ...
      <div class="mb-6 flex flex-wrap gap-2">
        {#each ["daily", "weekly", "monthly"] as tf}
          <button
            ...
            class="px-4 py-2 rounded-lg transition-colors capitalize {timeframe ===
            tf
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
        ...
        <Card>
          ...
          {:else}
            <div
              style="position: relative; height: 220px; width: 100%; display: flex; justify-content: center;"
            >
              <Pie
            ...
            {#snippet cell(row: any, column: any)}
              ...
              {:else if column.header === "Revenue"}
                <span class="font-medium text-green-600"
                  >₹{row.total_rev.toLocaleString()}</span
                >
              {:else}
                {row[column.accessor]}
              {/if}
