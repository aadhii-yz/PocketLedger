<script lang="ts">
  ...
  async function fetchBills(from: Date, to?: Date) {
    ...
    return pb.collection("bills").getFullList({ filter, sort: "-created", expand: "shop" });
  }
  ...
  const BILL_HEADERS = [
    "Bill Number", "Date", "Shop", "Customer", "Payment Method",
    "Payment Status", "Subtotal", "Tax", "Discount", "Grand Total",
  ];

  function financialSummary(bills: any[]): unknown[][] {
    const sum = (k: string) => bills.reduce((s, b) => s + (Number(b[k]) || 0), 0);
    ...
    for (const [m, e] of byMethod) rows.push([m, e.count, e.total.toFixed(2)]);
    return rows;
  }
  ...
  async function buildReport(id: string): Promise<{ name: string; csv: string }> {
    ...
    if (id === "stock") {
      const stock = await pb.collection("stock").getFullList({ expand: "product,location", sort: "location" });
      const rows = stock.map((s: any) => {
        ...
        return [
          p?.name || s.product, p?.sku || "", p?.barcode || "",
          s.expand?.location?.name || s.location,
          qty, threshold, price, (qty * price).toFixed(2),
      ...
      return {
        ...
        csv: toCSV(
          ["Product", "SKU", "Barcode", "Location", "Quantity", "Low Stock Threshold", "Selling Price", "Stock Value", "Status"],
          rows,
        ),
      };
    }
  ...
  async function runMonthlyDownload() {
    ...
    try {
      ...
      const csv = [
        ...
        toCSV(financialSummary(bills)[0] as string[], financialSummary(bills).slice(1)),
      ].join("\r\n");
      downloadCSV(
...
<div class="flex min-h-screen bg-background">
  ...
  <FluidLayout>
    ...
    {#if errorMsg}
      <div class="mb-6 flex items-center gap-2 p-3 bg-destructive/10 border border-destructive/30 rounded-lg text-destructive text-sm">
