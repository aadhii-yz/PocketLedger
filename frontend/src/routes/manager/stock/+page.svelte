<script lang="ts">
  ...
  import { LayoutDashboard, TrendingUp, Package, FileText, AlertTriangle, Archive, CheckCircle, Users } from 'lucide-svelte';
  import { pb } from '$lib/pb';
  import { onMount } from 'svelte';
  import { Chart, Title, Tooltip, Legend, CategoryScale, BarElement, ArcElement, LinearScale } from 'chart.js';
  import { Bar, Pie } from 'svelte-chartjs';

  Chart.register(Title, Tooltip, Legend, CategoryScale, BarElement, ArcElement, LinearScale);
  ...
  onMount(() => {
    async function loadData() {
      try {
        ...
        const priceMap = new Map((productRecords as any[]).map((p) => [p.id, p.selling_price || 0]));
        ...
        for (const s of stockRecords as any[]) {
          const catName = s.expand?.product?.expand?.category?.name || 'Uncategorized';
          const qty: number = s.quantity || 0;
          const threshold: number = s.low_stock_threshold || 0;
          const price = priceMap.get(s.product) || 0;
          stockValue += qty * price;

          if (!catMap[catName]) catMap[catName] = { category: catName, total: 0, low: 0, healthy: 0 };
        ...
        lowStockItems = lowItems.sort((a, b) => a.currentStock - b.currentStock);
...
<div class="flex min-h-screen bg-background">
  ...
  <FluidLayout>
    {#if loading}
      <LoadingSpinner />
    {:else}
      <PageHeader title="Stock Analysis" subtitle="Inventory levels and stock monitoring" icon={Package} />

      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
        <StatCard title="Total Items" value={totalStock} change="All categories" icon={Archive} trend="neutral" delay={0} />
        <StatCard title="Healthy Stock" value={totalHealthy} change={totalStock ? `${Math.round((totalHealthy / totalStock) * 100)}% of total` : '—'} icon={CheckCircle} trend="up" delay={0.1} />
        <StatCard title="Low Stock" value={totalLowStock} change="Need reorder" icon={AlertTriangle} trend="down" delay={0.2} />
        <StatCard title="Stock Value" value={`₹${(totalStockValue / 1000).toFixed(0)}K`} change="Total inventory value" icon={Package} trend="neutral" delay={0.3} />
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
        <Card>
          <h3 class="mb-4 text-lg md:text-xl">Stock Distribution by Category</h3>
          ...
          <div class="overflow-x-auto">
            {#snippet overviewCell(row: any, column: any)}
              ...
              {:else if column.header === 'Healthy'}
                <span class="text-green-600 font-medium">{row.healthy} units</span>
              {:else if column.header === 'Low Stock'}
                <span class={row.low > 0 ? 'text-destructive font-medium' : 'text-muted-foreground'}>{row.low} units</span>
              {:else if column.header === 'Health %'}
                {@const pct = row.total ? Math.round((row.healthy / row.total) * 100) : 100}
                <div class="flex items-center gap-2">
                  <div class="w-24 h-2 bg-muted rounded-full overflow-hidden">
                    <div class="h-full {pct > 80 ? 'bg-green-600' : pct > 50 ? 'bg-yellow-500' : 'bg-red-600'}" style="width: {pct}%"></div>
                  </div>
      ...
      <Card>
        ...
        {#if lowStockItems.length === 0}
          <p class="text-center py-8 text-green-600 font-medium">✓ All items are adequately stocked</p>
        {:else}
          <div class="overflow-x-auto">
            {#snippet lowCell(row: any, column: any)}
              {#if column.header === 'Current Stock'}
                <span class="text-destructive font-medium">{row.currentStock} units</span>
              ...
              {:else if column.header === 'Priority'}
                ...
                {@const priority = ratio <= 0.3 ? 'Critical' : ratio <= 0.6 ? 'High' : 'Medium'}
                {@const color = priority === 'Critical' ? 'bg-red-100 text-red-700' : priority === 'High' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'}
                <span class="px-2 py-1 rounded text-xs font-medium {color}">{priority}</span>
              {:else}
                {row[column.accessor]}
              {/if}
            {/snippet}
