...
- `lib/services/printer_discovery.dart` — auto-detection singleton (`ChangeNotifier`). Detection chain per platform:
  ...
  - **Windows:** Named USB printer via PowerShell `Win32_Printer` (matches `Name` against `3230`/`rp3230` for receipt, `lp46`/`dlite` for label — mirrors Linux CUPS matching) → generic USB port fallback → TCP scan
