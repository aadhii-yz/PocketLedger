...
- `lib/services/printer_discovery.dart` — auto-detection singleton (`ChangeNotifier`). Detection chain per platform:
  ...
  - **Windows:** Named USB printer via PowerShell `Win32_Printer` (matches `Name` against `3230`/`rp3230` for receipt, `lp46`/`dlite` for label — mirrors Linux CUPS matching) → generic USB port fallback → TCP scan. Requires printers to be installed in Windows first (Settings → Printers & scanners → Add device; use "Generic / Text Only" driver and name exactly `TVS RP 3230` / `TVS LP 46`). When no match is found, `Get-PnpDevice` is also run and logged to show physically connected devices.
