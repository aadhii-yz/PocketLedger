...
- `src/lib/print.ts` — Print utility: `loadPrintSettings()` (fetches singleton from `print_settings` collection), `printReceipt(bill, settings)` (80mm thermal receipt), `printBarcode(product, settings)` (single barcode label), `listQZPrinters()` (returns all printers visible to QZ Tray). Both print functions follow a **four-step fallback chain**: (1) Tauri IPC (`window.__TAURI__.core.invoke`) — only available when the WebView is on `tauri://localhost`; NOT available when the WebView has navigated to a remote PocketLedger URL (no `dangerousRemoteDomainIpcAccess` configured); (2) companion app HTTP at `localhost:8765` — 800 ms availability check, then POST — this is the effective step for billing/stock users in the companion WebView; (3) QZ Tray raw/HTML print (if a printer name is set in `print_settings`); (4) `window.open` + browser print dialog. QZ Tray is lazy-loaded via dynamic `import('qz-tray')` and uses unsigned security mode. Barcode PNG is fetched from `/api/custom/barcode/{id}` and converted to a base64 data URL. `PrintSettings` type re-exported from `schemas.ts`.
- `src/lib/companion.ts` — Companion mode helpers. `isCompanionMode()` reads `sessionStorage` for the `companion_mode` key (set by `+layout.svelte` when `?companion=1` is in the URL). `companionMenuItem` is a pre-built `ImprovedSidebar` menu item ("Printer Settings", Settings icon) whose `onclick` navigates `window.location.href` to `tauri://localhost`, taking the WebView back to the companion settings page.
...
- `src/lib/components/` — shared UI primitives (Button, Card, DataTable, etc.). `ImprovedSidebar.svelte` accepts `menuItems: { label, icon, path, onclick? }[]` — when `onclick` is set on an item, it is called instead of `goto(path)` and the item is never marked as the active route.
...
**Companion mode detection:** The root `+layout.svelte` checks for `?companion=1` in the URL on mount and stores `companion_mode=1` in `sessionStorage`. All billing and stock pages read this via `isCompanionMode()` from `$lib/companion` and append `companionMenuItem` to their sidebar menu items when true. `sessionStorage` scope (single tab/window) prevents companion mode from bleeding into regular browser tabs. The `?companion=1` param is appended by the companion app when navigating to billing/stock routes.
...
Tauri v2 app (plain Vite+Svelte + Rust) that serves two purposes: (1) navigates its WebView to the configured PocketLedger URL for billing/stock routes, giving those users a native app experience; and (2) runs a local HTTP server on `localhost:8765` so browser-based users can also reach the LAN printers. Required because the backend can be cloud-hosted (can't reach LAN printers) and browsers can't open raw TCP sockets.
...
**First-time device setup:** Open companion app → enter PocketLedger URL + printer IPs → Save → click "Open Billing" or "Open Stock Entry". Settings are persisted to Tauri's app data dir. The Rust HTTP server starts automatically; on Android the foreground service starts on app open. For WebView use (billing/stock users), printing goes via `localhost:8765` (step 2 of fallback chain — Tauri IPC is not available on the remote URL). For browser use (manager/admin), `print.ts` pings `localhost:8765/status` first and falls through to QZ Tray or browser print if the companion is not running.
