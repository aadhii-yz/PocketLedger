...
- `stock_entry` **cannot** update `stock` quantities directly via the PocketBase REST API (`PATCH /api/collections/stock/records/{id}`) — the collection `updateRule` is admin/manager only. All stock quantity changes must go through `POST /api/custom/stock/adjust`, which writes the `stock_movements` audit row.
